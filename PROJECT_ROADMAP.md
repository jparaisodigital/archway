# Archway Modern Website – Updated Roadmap

**Client:** Archway International & Marketing Services, Inc.  
**Fee:** ₱15,000  
**Stack:** Vanilla JS + Tailwind CSS (CDN) + Config-driven  
**Application Email:** hr@archwayintl.com.ph  
**Realistic Timeline:** 5–7 days

---

## Immediate Priority (Today – for Meeting)

**Goal:** Working Application Form that sends to HR email (within 2 hours)

- [x] Create simplified but long Application Form
- [x] Include Resume / CV file upload
- [x] Connect to Web3Forms or Getform
- [x] Test sending to hr@archwayintl.com.ph
- [x] Make it look clean & modern

**This is what you will present in the meeting.**

---

## Phase 1: Foundation
- [x] Project setup
- [x] Config system
- [x] Logo
- [ ] Shared Navbar & Footer
- [ ] Design system (colors, spacing, typography)

---

## Phase 2: Home Page
- [ ] Modern Hero
- [ ] Local & Overseas Jobs section
- [ ] Welcome / About preview
- [ ] CTAs

---

## Phase 3: Applicants Page
- [ ] Application Form (priority – working email)
- [ ] Job Interview Protocols
- [ ] Documentary Requirements
- [ ] Process Steps (“We’ll Be There Every Step of the Way”)
- [ ] Contact section

---

## Phase 4: Employers Page
- [ ] Documentary Requirements list
- [ ] Contact / Partner form

---

## Phase 5: About Us Page
- [ ] Company story
- [ ] Contact section

---

## Phase 6: Jobs Management (Simplified)
- [ ] Use **Google Sheets** as the source of jobs
- [ ] Display Local & Overseas jobs from the Sheet
- [ ] Client can edit jobs directly in Google Sheets (no custom admin needed)
- [ ] Optional: Simple “Edit Jobs” button that opens the Sheet

---

## Phase 7: Polish & Handover
- [ ] Responsive testing
- [ ] Form testing
- [ ] 1–2 revision rounds
- [ ] Handover files + instructions
- [ ] Final payment

---

## Out of Scope (Not included in ₱15,000)
- Real database / applicant tracking system
- Multi-user admin with roles
- Automated File ID system (like the old site)
- Payment gateway
- Ongoing maintenance
- Domain & hosting setup (unless simple)
- Advanced SEO or marketing tools

---

## Tech Decisions (Locked)
- Multi-page (Home, Applicants, Employers, About Us)
- Vanilla JS + Tailwind CDN
- Config-driven content
- Form → Web3Forms / Getform (file upload supported)
- Jobs → Google Sheets (simple & future-proof)
- Future-ready for Supabase if needed later